...
int main()
{
    ...
    RectangleShape boundaryLeft, boundaryRight, boundaryTop, boundaryBottom;
    boundaryLeft.setSize(Vector2f(10.f, 3360.f));
    boundaryLeft.setFillColor(sf::Color::Blue);
    ...
    while (app.isOpen())
    {
        ...
        if (car[0].x < 0)
            car[0].x = 0;
        else if (car[0].x > 1440)
            car[0].x = 1440;

        if (car[0].y < 0)
            car[0].y = 0;
        else if (car[0].y > 1824)
            car[0].y = 1824;
        ...
        boundaryLeft.setPosition(130-offsetX,140-offsetY);
        app.draw(boundaryLeft);
        for(int i=0;i<N;i++)
        {
